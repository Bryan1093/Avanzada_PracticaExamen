...
import com.example.examen.utilidades.Funciones;
...
//---------------------------------------------------------------------------------------
        ...
        for (int i = 0; i < segmentos; i++) {
            coloresAL.add(1.0f);coloresAL.add(1.0f);coloresAL.add(1.0f);coloresAL.add(1.0f);//GRIS
    ...
    public void dibujar(GL10 gl){
        gl.glFrontFace(gl.GL_CCW);  // Set the front face to counterclockwise
        ...
        gl.glPointSize(12);
        gl.glLineWidth(12);
        //gl.glDrawArrays(gl.GL_LINE_LOOP, 0, verticesAL.size() / 2);
        gl.glDrawArrays(gl.GL_TRIANGLE_FAN, 0, verticesAL.size() / 2);


        gl.glFrontFace(gl.GL_CCW);  // Reset the front face to its original state
